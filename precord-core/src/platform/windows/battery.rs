    ...
    pub fn new() -> Result<Option<Self>, Error> {
        unsafe {
            ...
            let pdidd = &mut *(local
                .0
                 .0
                .cast::<DeviceAndDriverInstallation::SP_DEVICE_INTERFACE_DETAIL_DATA_W>(
            ));
...
impl Drop for OwnedDeviceInfo {
    fn drop(&mut self) {
        unsafe {
            let _ = DeviceAndDriverInstallation::SetupDiDestroyDeviceInfoList(self.0);
        }
    }
}

struct OwnedLocalMemory(Foundation::HLOCAL);

impl Drop for OwnedLocalMemory {
    fn drop(&mut self) {
        unsafe {
            let _ = Foundation::LocalFree(self.0);
        }
    }
}
